...
dodCount = 15;    % number of points used on the DOD discretization. IT'S VERY VERY ADVISABLE TO USED AN ODD NUMBER
...
dayHour = cumsum(modelV2.read('dt'));
[demandas, ~] = modelV2.lookup('map_th', 'demandas_contratadas');

venda  = modelV2.read('prv', 't');
compra = modelV2.read('prc', 't');
compraLiq = compra - venda;
soc = modelV2.read('soc1', 't');

[my_figure, my_plot ]= my_line_plot(dayHour, soc, 'SOC');
% my_figure = my_stairs_plot(dayHour, soc, 'compra MR');
